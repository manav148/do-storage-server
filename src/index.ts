...
class DOStorageServer {
  ...
  private async handleUpload(args: any) {
    if (!args.key || !args.content) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Missing required parameters: key and content'
      );
    }
  ...
  private async handleUploadFile(args: any) {
    if (!args.key || !args.filepath) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Missing required parameters: key and filepath'
      );
    }
  ...
  private async handleList(args: any) {
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `List failed: ${(error as Error).message}`
      );
    }
  }
